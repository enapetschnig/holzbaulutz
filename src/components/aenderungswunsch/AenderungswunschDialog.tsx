...
/**
 * Die Tabelle steht (noch) nicht in types.ts — im Baukasten übliches
 * Cast-Muster. `supabase.storage.from("aenderungswuensche")` bleibt
 * ungecastet: Storage ist untypisiert und heißt nur zufällig gleich.
 */
...
export function AenderungswunschDialog({ open, onOpenChange, bild, seite }: Props) {
  ...
  const senden = async () => {
    ...
    try {
      ...
      const { data: neu, error } = await aenderungTable()
        .insert({
