...
/** Tabelle fehlt in types.ts — Cast-Muster wie im Rest des Baukastens. */
...
export function AenderungswuenscheListe() {
  ...
  const setzen = async (id: string, felder: Partial<Wunsch>) => {
    const { error } = await aenderungTable().update(felder).eq("id", id);
  ...
  const loeschen = async (w: Wunsch) => {
    ...
    const { error } = await aenderungTable().delete().eq("id", w.id);
    setArbeitet(null);
