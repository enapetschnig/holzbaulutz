-- Änderungswünsche aus der App: melden, ansehen, beantworten.
--
-- Wer in der App auf "Änderung melden" tippt, legt hier eine Zeile an —
-- mit Bildschirmfoto und optionaler Sprachnachricht im privaten Bucket
-- `aenderungswuensche`. Die Abschrift der Sprachnachricht trägt die Edge
-- Function `sprache-zu-text` nach (Feld `abschrift`: offen → laeuft →
-- fertig/fehler); niemand wartet im Dialog darauf.
